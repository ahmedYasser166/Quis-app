...
export default class Question {
  constructor(index) {
    ...
    this.answers = [
      AllQuestionsArr[index].correct_answer,
      ...AllQuestionsArr[index].incorrect_answers,
    ].sort();
  ...
  displayQuestion() {
    ...
    Questions__Container.innerHTML = `
      ...
      <div>
      <p>Current Question <span class="badge bg-primary">${
        this.index + 1
      }</span></p>
      <p>Count of Question <span class="badge bg-primary">${
        AllQuestionsArr.length
      }</span></p>
      </div>
  ...
  HandleChoices() {
    return this.answers
      .map(
        (answer) =>
          `<li class="col-6"><button class="answer w-100 h-100 btn btn-outline-dark">${answer}</button></li>`
      )
      .join("");
  }
  ...
  displayFinalResult() {
    Questions__Container.innerHTML = `
      ...
      <div class="card-footer"><span class="badge bg-warning">Your Score Is: ${
        quiz.score === AllQuestionsArr.length
          ? "Congratulations ✅🎉🎉"
          : quiz.score
      }</span></div>
